...
With `cs_get_n()` one can extract the number of participants used in a
clinical significance analysis from a `cs_analysis`object. This may
depend on the clinical significance approach and if missing values were
present in the dataset. For all individual analyses, missing values are
handled by list-wise deletion. Consequently, individuals with a missing
pre or post intervention score will be omitted from the analyses.
...
- which:
  ...
  - `"all"`, (the default) returns the number of participants in both,
    the original and used data set

  - `"original"`, number of participants in the original dataset

  - `"used"`, number of participants in the used data set, so after
    conversion to wide format and omitting cases with missing values
